...
class BeschichtungDetector:
    COLORS_FILE = "beschichtung.json"  # Datei zur Speicherung der Farben

    def __init__(self):
        self.unidekore = self.load_colors()

    def load_colors(self) -> Dict[str, BeschichtungFarbe]:
        """Lädt die Farben aus der JSON-Datei"""
        if os.path.exists(self.COLORS_FILE):
            with open(self.COLORS_FILE, "r", encoding="utf-8") as file:
                data = json.load(file)
                return {name: BeschichtungFarbe(**details) for name, details in data.items()}
        return {}
    ...
    def add_color(self, name: str, hex_code: str, ncs_code: str):
        ...
        self.unidekore[name] = BeschichtungFarbe(
            name=name,
            hex_code=hex_code,
            ncs_code=ncs_code,
